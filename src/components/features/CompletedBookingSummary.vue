<template>
  <div class="max-w-6xl mx-auto space-y-8">
    ...
    <div class="bg-gradient-to-r from-green-50 to-emerald-50 dark:from-green-950/20 dark:to-emerald-950/20 rounded-xl p-6 border border-green-200 dark:border-green-800">
        ...
        <CardContent class="p-6">
          <div class="flex items-center justify-between">
            <div>
              ...
              <p class="text-lg font-semibold text-gray-900 dark:text-white">{{ details.customer_name }}</p>
        ...
        <CardContent class="p-6">
          <div class="flex items-center justify-between">
            <div>
              ...
              <p class="text-lg font-semibold text-gray-900 dark:text-white">{{ details.vehicle.name }}</p>
              <p class="text-sm text-gray-500 dark:text-gray-400">{{ details.vehicle.brand }} {{ details.vehicle.model }}</p>
        ...
        <CardContent class="p-6">
          <div class="flex items-center justify-between">
            <div>
              ...
              <p class="text-lg font-semibold text-gray-900 dark:text-white">₱{{ Number(details.security_deposit).toLocaleString() }}</p>
              <p class="text-sm text-gray-500 dark:text-gray-400">{{ details.security_deposit_words }}</p>
            </div>
        ...
        <CardContent class="space-y-4">
          <div class="grid grid-cols-2 gap-4">
            <div class="space-y-1">
              ...
              <p class="text-lg font-semibold">₱{{ Number(details.daily_rate).toLocaleString() }}</p>
        ...
        <CardContent>
          <div class="space-y-3">
            <div class="flex justify-between items-center p-3 bg-gray-50 dark:bg-gray-800 rounded-lg">
              ...
              <span class="text-lg font-bold text-green-600">₱{{ Number(Number(details.rental_amount || 0) + Number(details.vehicle_return?.late_fee || 0) + Number(details.vehicle_return?.damage_fee || 0) + Number(details.vehicle_return?.cleaning_fee || 0)).toLocaleString() }}</span>
            </div>
            <div class="text-sm text-gray-600 dark:text-gray-400">
              {{ details.payments.length }} payment{{ details.payments.length !== 1 ? 's' : '' }} recorded
            </div>
          </div>
        </CardContent>
      </Card>
    ...
    <Card>
      <CardHeader>
        ...
        <p class="text-sm text-gray-600 dark:text-gray-400">
          {{ details.payments.length }} payment{{ details.payments.length !== 1 ? 's' : '' }} processed
        </p>
      </CardHeader>
      <CardContent>
        <div class="space-y-4">
          ...
          >
            <div class="flex-shrink-0">
              <div class="w-10 h-10 rounded-full flex items-center justify-center"
                   :class="payment.type === 'deposit' ? 'bg-amber-100 text-amber-600' : 'bg-blue-100 text-blue-600'">
            ...
            <div class="flex-1 min-w-0">
              <div class="flex items-center justify-between mb-2">
                <div class="flex items-center gap-2">
                  ...
                  <Badge 
                    :variant="payment.status === 'approved' ? 'success' : payment.status === 'pending' ? 'warning' : 'destructive'"
                    class="text-xs"
                  >
                    {{ payment.status }}
                  </Badge>
              ...
              <div class="grid grid-cols-1 md:grid-cols-2 gap-2 text-sm text-gray-600 dark:text-gray-400 mb-3">
                ...
                <div><span class="font-medium">Reference:</span> {{ payment.reference_number }}</div>
                <div><span class="font-medium">Created:</span> {{ formatDate(payment.created_at) }}</div>
                <div><span class="font-medium">Approved:</span> {{ payment.approved_at ? formatDate(payment.approved_at) : '—' }}</div>
              </div>
        ...
        <CardContent class="space-y-4">
          <div class="grid grid-cols-2 gap-4">
            <div class="space-y-1">
              ...
              <p class="text-base font-semibold">{{ formatDate(details.vehicle_release.released_at) }}</p>
            </div>
            <div class="space-y-1">
          ...
          <div v-if="details.vehicle_release.condition_notes" class="space-y-1">
            ...
            <p class="text-base bg-gray-50 dark:bg-gray-800 p-3 rounded-lg">{{ details.vehicle_release.condition_notes }}</p>
          </div>
          
          <div v-if="details.vehicle_release.images && details.vehicle_release.images.length" class="space-y-2">
        ...
        <CardContent class="space-y-4">
          <div class="grid grid-cols-2 gap-4">
            <div class="space-y-1">
              ...
              <p class="text-base font-semibold">{{ formatDate(details.vehicle_return.returned_at) }}</p>
            </div>
            <div class="space-y-1">
              <p class="text-sm font-medium text-gray-600 dark:text-gray-400">Odometer</p>
          ...
          <div class="grid grid-cols-2 gap-4">
            ...
            <div class="space-y-1">
              ...
              <p class="text-base font-semibold text-red-600">₱{{ Number(details.vehicle_return.late_fee).toLocaleString() }}</p>
            </div>
          </div>
          ...
          <div class="grid grid-cols-2 gap-4">
            <div class="space-y-1">
              ...
              <p class="text-base font-semibold text-red-600">₱{{ Number(details.vehicle_return.damage_fee).toLocaleString() }}</p>
            </div>
            <div class="space-y-1">
              <p class="text-sm font-medium text-gray-600 dark:text-gray-400">Cleaning Fee</p>
              <p class="text-base font-semibold text-red-600">₱{{ Number(details.vehicle_return.cleaning_fee).toLocaleString() }}</p>
            </div>
          </div>
          ...
          <div v-if="details.vehicle_return.condition_notes" class="space-y-1">
            ...
            <p class="text-base bg-gray-50 dark:bg-gray-800 p-3 rounded-lg">{{ details.vehicle_return.condition_notes }}</p>
          </div>
          
          <div v-if="details.vehicle_return.images && details.vehicle_return.images.length" class="space-y-2">
